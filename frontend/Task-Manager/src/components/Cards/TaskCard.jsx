...
const TaskCard = ({ title, description, priority, status, progress, createdAt, dueDate, assignedTo, attachmentCount, completedTodoCount, todoChecklist, onClick }) => {
    // Validate and calculate progress if not provided
    const calculatedProgress = progress ?? 
        (todoChecklist?.length > 0 ? Math.round((completedTodoCount / todoChecklist.length) * 100) : 0)
    const safeProgress = Math.min(Math.max(Number(calculatedProgress) || 0, 0), 100)

    // Normalize status to match Progress component cases
    const normalizedStatus = status ? status.charAt(0).toUpperCase() + status.slice(1).toLowerCase() : 'Not started'
    ...
    return (
        ...
        >
            ...
            <div className={`px-4 border-l-[3px] ${normalizedStatus === 'In Progress' ? 'border-cyan-500' : normalizedStatus === 'Completed' ? 'border-indigo-500' : 'border-violet-500'}`}>
                ...
                <Progress progress={safeProgress} status={normalizedStatus} />
            </div>

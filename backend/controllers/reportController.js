...
const exportUsersReport = async (req, res) => {
  try {
    ...
    tasks.forEach((task) => {
      console.log(
        `Task ${task._id}: status=${task.status}, assignedTo=${
          task.assignedTo
            ? task.assignedTo.map((u) => u._id).join(", ")
            : "null"
        }`
      );
      ...
      task.assignedTo.forEach((user) => {
        ...
        if (userTaskMap[userId]) {
          ...
          console.log(
            `Incremented taskCount for user ${userId}: ${userTaskMap[userId].taskCount}`
          );
          ...
          switch (status) {
            case "pending":
              ...
              console.log(
                `Incremented pendingTasks for user ${userId}: ${userTaskMap[userId].pendingTasks}`
              );
              break;
            case "in progress":
              userTaskMap[userId].inProgressTasks += 1;
              console.log(
                `Incremented inProgressTasks for user ${userId}: ${userTaskMap[userId].inProgressTasks}`
              );
              break;
            case "completed":
              userTaskMap[userId].completedTasks += 1;
              console.log(
                `Incremented completedTasks for user ${userId}: ${userTaskMap[userId].completedTasks}`
              );
              break;
            default:
              console.warn(
                `Task ${task._id} has invalid status: ${task.status}`
              );
          }
        } else {
          console.warn(
            `User ${userId} not found in userTaskMap for task ${task._id}`
          );
        }
      });
    });
